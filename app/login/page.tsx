...
import { useAuth } from '../context/AuthContext'; // useAuth ইম্পোর্ট করা হয়েছে

export default function LoginPage() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const { login } = useAuth(); // login ফাংশন আনা হয়েছে
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      setError('Login failed. Please check your credentials.'); // ব্যর্থ হলে ত্রুটি বার্তা
    }
  };
